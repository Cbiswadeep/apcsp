function accuracy  = evaluateRCSPAlgo_var(algo, dataSet, printTopo, ord)
%
...
low = 8;
high = 30;
order = ord; %we use a 5th-order butterworth filter
...
for s=1:nbSubjects    
    ...
    if strcmp(algo,'CSP')
     ...
     elseif strcmp(algo,'CSPPhaseAmpJ')
         disp('Learning CSP filters with joint diagonalization');
         CSPMatrix = learnCSPLagrangian(trainingEEGSignals{s});
...
%         CSPMatrix = learn_CCSP2(trainingEEGSignals{s}, trainingEEGSignals(otherSubjects), bestBeta);
    ...
    if printTopo==1
        ...
        elecIndex = ismember(emap(1,:),trainingEEGSignals{1,1}.c);    
        emap = emap(:,elecIndex);        
        for f=1:nbFilterPairs
            plotElecPotentials(emap,CSPMatrix(f,:));
%             filenameFig = ['' root 'S1'  '_FilterPair' num2str(f) '_C1.fig'];
%             filenameEPS = ['' root 'S1' '_FilterPair' num2str(f) '_C1.eps'];
%             filenameJPG = ['' root 'S1' '_FilterPair' num2str(f) '_C1.jpeg'];            
%             print(filenameFig);
